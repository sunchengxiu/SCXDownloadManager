...
#import "SCXSessionViewController.h"
...
@interface SCXSessionViewController ()

@end

@implementation SCXSessionViewController

-(void)viewDidLoad{
    [super viewDidLoad];
    self.urlArr = @[downloadUrl , downloadUrl1 , downloadUrl2];
    for (NSString *url in self.urlArr) {
        SCXDownloadModel *model = [self getSessionDownloadTaskProgressInfo:url];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    SCXSessionCell *cell = [tableView dequeueReusableCellWithIdentifier:sessionCellID];
    if (cell == nil) {
        cell = [[SCXSessionCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:sessionCellID];
    }
    [cell setModel:self.modelArr[indexPath.row]];
    cell.sessionCellDelegate = self;
    return cell;
    
}
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    //动态获取cell的高度，进行设置
    SCXSessionCell *cell = [self tableView:tableView cellForRowAtIndexPath:indexPath];
    [cell layoutIfNeeded];
...
-(void)updateSessionCell:(SCXTableViewCell *)cell progress:(SCXDownloadProgressModel *)progress{
    ...
    SCXSessionCell *cell1 = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
    BOOL reload = NO;
